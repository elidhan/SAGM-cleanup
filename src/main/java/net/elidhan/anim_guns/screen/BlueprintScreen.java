...
public class BlueprintScreen extends HandledScreen<BlueprintScreenHandler>
{
    ...
    protected void init()
    {
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal("Set"), (button) ->
        {
            if (client != null)
            {
                ...
                ClientPlayNetworking.send(ModNetworking.C2S_SELECT_BLUEPRINT, buf);
    ...
    {
        Text text = Text.translatable(BlueprintItem.BLUEPRINT_ITEM_LIST.get(getCurrentBlueprint()).getTranslationKey());

        context.drawText(this.textRenderer, text, (context.getScaledWindowWidth()/2)-((this.width-this.backgroundWidth)/2)-(this.textRenderer.getWidth(text)/2), (context.getScaledWindowHeight()/2)-((this.height-this.backgroundHeight)/2)-36,16777215,true);
    }
